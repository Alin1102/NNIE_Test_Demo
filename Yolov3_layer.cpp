            ...
            for (int cx = 0; cx < Box_w; cx++) {
                for (int box = 0; box < BoxNum; box++) {
                    ...
                    if (tc > ConfThresInt) {
                        ...
                        float x = (cx + Sigmoid(float(tx)/4096.0)) / Box_w;
                        float y = (cy + Sigmoid(float(ty)/4096.0)) / Box_h;
                        float w = anchors[box].width * exp(float(tw)/4096.0);
                        float h = anchors[box].height * exp(float(th)/4096.0);                       
